...
const HeaderMenu = ({ basketCount, isGoBackVisible }: HeaderMenuProps) => {
  ...
  return (
    <div className='fixed z-40 top-0 left-0 w-full bg-zinc-900'>
      <div className='container mx-auto max-lg:max-w-none flex justify-end items-center xs:gap-x-8 gap-x-4 px-5 py-3'>
        <div className='flex-1 justify-self-start flex justify-start'>
          {isGoBackVisible
            ? <NavLink to='/' className=''>
              <ArrowBackIcon className='xs:h-12 xs:w-12 h-10 w-10 rotate-180 fill-white px-2 bg-gray-500 rounded-full' />
            </NavLink>
        ...
        <NavLink to='/basket' className='relative'>
          <BasketIcon className='xs:h-12 xs:w-12 h-10 w-10 p-2 bg-red-500 rounded-full fill-white' />
          <div className='absolute -top-1 xs:-right-2 -right-3 bg-white text-red-500 px-2 rounded-full font-medium'>
